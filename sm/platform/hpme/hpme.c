...
enclave_ret_code platform_init_global_once(){
  ...
  printm("[SM] Platform init global once success.\n");
  return ENCLAVE_SUCCESS;
}

enclave_ret_code platform_init_global(){
  pmp_set(scratch_rid, PMP_NO_PERM);
  printm("[SM] Platform init global success.\n");
...
enclave_ret_code platform_create_enclave(struct enclave* enclave){
  ...
  enclave->ped.key[0] = platform_random();
  enclave->ped.key[1] = platform_random();
  enclave->ped.firstPageInitCounter[0] = 0x10000;
  enclave->ped.firstPageInitCounter[1] = 0;
  ...
  size_t size = enclave->pa_params.free_base - scratch_epm_start;
  size_t scratch_size = L2_SIZE;

  if(size > scratch_size){
    printm("[SM]FATAL: Enclave too big for scratchpad!\r\n");
    return ENCLAVE_NO_FREE_RESOURCE;
  }
  memcpy((enclave_ret_code*)scratch_epm_start,
         (enclave_ret_code*)old_epm_start,
         size);
  printm("[SM]Performing copy from %llx to %llx\r\n", old_epm_start, scratch_epm_start);
  ...
  // change page table to scratch
  //pte_t* walk;
  //for(walk=(pte_t*)scratch_epm_start;walk < (pte_t*)(scratch_epm_start + (RISCV_PGSIZE/sizeof(pte_t)));walk += 1){
  //  *walk = *walk - ((old_epm_start-scratch_epm_start) >> 2);
  //}
  
 // printm("[DEBUG] enclave->encl_satp=0x%lx\n", enclave->encl_satp);

 // printm("[SM][new pa_params]: \r\n\tbase_addr: %llx\r\n\tbasesize: %llx\r\n\truntime_addr: %llx\r\n\tuser_addr: %llx\r\n\tfree_addr: %llx\r\n", 
